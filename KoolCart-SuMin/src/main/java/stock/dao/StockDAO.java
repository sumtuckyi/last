...
//    List<MainDTO> findAllPersons();
    ...
    @Query(value =
            "SELECT "+
                ...
                "GROUP BY ivt.user_seq, ivt.sctg_seq, ivt.pdt_seq, ivt.name " +
                "ORDER BY " +
                "CASE " +
                	"WHEN :ctgType = 0 THEN MIN(ivt.exp) " +
                	"WHEN :ctgType = 1 THEN MIN(ivt.ivt_seq) " +
                	"WHEN :ctgType = 2 THEN ivt.name " +
                "ELSE MIN(ivt.exp) END"
            , nativeQuery = true)
...
//                ", ivt.pdt_seq " +
    ...
    @Query("SELECT st FROM StockDTO st WHERE st.user_seq = :userSeq AND st.pdt_seq = :pdtSeq AND st.favorite = true")
    List<StockDTO> stock_total_favorite(
    		@Param("userSeq") int userSeq,
    		@Param("pdtSeq") int pdtSeq
    		);
    ...
    "SELECT "+
        ...
        "LEFT JOIN sub_category sc on ivt.sctg_seq = sc.sctg_seq " +
        "LEFT JOIN main_category mc on sc.mctg_seq = mc.mctg_seq " +
        "WHERE ivt.user_seq = :userSeq AND mc.mctg_seq = :mctgSeq"
    , nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO message(mc_seq, user_seq, ivt_seq, logtime) values(3, :userSeq, :ivtSeq, NOW());", nativeQuery = true)
	void stock_message_exp(@Param("userSeq") int userSeq, @Param("ivtSeq") int ivtSeq);

    @Modifying
    @Transactional
    @Query(value = "INSERT INTO inventory(user_seq, sctg_seq, pdt_seq, name, count, exp) values(:#{#pdt.user_seq}, :#{#pdt.sctg_seq}, :#{#pdt.pdt_seq}, :#{#pdt.name}, :#{#pdt.count}, now() + 7)", nativeQuery = true)
