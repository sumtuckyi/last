...
export default function Message({ orderId, orderDate, user, alarmId, func }) {
  ...
  let postData = []
  // "[{user_seq: 3, sctg_seq:3, pdt_seq: 3, name: '사과', count:1}, 
  // {user_seq: 3, sctg_seq:3, pdt_seq: 3, name: '사과', count:1}, 
  // {user_seq: 3, sctg_seq:3, pdt_seq: 3, name: '사과', count:1}]"
  const addItems = async () => {
    postData = products.map((item) => ({
      user_seq: userSeq,
      sctg_seq: item.sctg_seq,
      pdt_seq: item.pdt_idx,
      name: item.name,
      count: item.count,
    }));
    await axios.post('api/stock/message/add',
      {
        pdt: postData
      })
    ...
    <div>
      <Collapse
        items={[
          {
            key: '1',
            label: `${orderDate}에 주문하신 상품이 도착하였습니다. ${alarmId}`, // 알림 제목 - 3월 1일에 주문하신 상품이 배송 완료되었습니다.
            children: (
              <MessageBox>
                {/* 알림 내용 */}
