...
const NotificationCenter = () => {
  ...
  function calculateTimeLeft(expiryDate){
      ...
      if (difference > 0) {
        ...
        return `${timeLeft.days}일 ${timeLeft.hours}시간 ${timeLeft.minutes}분`; 
      }
  ...
  const fetchNotifications = async () => {
    try {
      const response = await PrevService.getAllAlarms(userId);
      const alarmData = response.data;
      setNotifications(alarmData.reverse());
    } catch (error) {
    ...
    console.log(notification);
      ...
      case 1:
        ...
        >
          ...
          >
            <Typography sx={{fontSize: "0.5em"}}>주문번호 : {notification.orderId}</Typography> 
            <Typography sx={{fontSize: "0.5em"}}>배송 상태 :{' '}
              {notification.delivered ? '배송완료' : '배송 중'}</Typography>
          </Box>
        </Box>;
      case 2:
        return <Box className="Prev-notification-content-box"
        ...
        <Box className="Prev-notification-content-summary"
          ...
          >
        <Typography sx={{fontSize: "0.5em"}}>상품: {notification?.regularId?.name}</Typography>
        <Typography sx={{fontSize: "0.5em"}}>예정일 : {notification?.regularId?.nextOrder} (1일 남음)</Typography>
        </Box>
      </Box>;
      case 3:
        return <Box className="Prev-notification-content-box"
        ...
        <Box className="Prev-notification-content-summary"
          ...
          >
        <Typography sx={{fontSize: "0.5em"}}>유통기한 임박 품목 : {notification?.itemId?.name}</Typography>
        <Typography sx={{fontSize: "0.5em"}}>잔여 유통기한 : {calculateTimeLeft(notification?.itemId?.exp)}</Typography>
        </Box>
      </Box>;
      default:
        return '';
